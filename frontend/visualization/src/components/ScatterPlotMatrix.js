...
import axios from 'axios';
 ...
 const ScatterPlotMatrix = (props) => {
    ...
    useEffect(()=>{
        ...
        axios.get('http://localhost:8000/loading/'+(props.dimensionality?props.dimensionality:4)).then((repos) => {
        ...
        axios.get('http://localhost:8000/scatterplotmatrix').then((repos) => {
            const allRepos = repos.data;
            let data= JSON.parse(allRepos);
            let temp=data;
